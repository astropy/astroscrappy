...
def detect_cosmics(indat, inmask=None, bkg=None, var=None, float sigclip=4.5,
    ...
    bkg : float numpy array, optional
        A pre-determined background image, to be subtracted from ``indat``
        before running the main detection algorithm (after deriving any noise
        estimates). This is used primarily with spectroscopic data, to remove
        ...
        (`cleanarr`). The necessary input array can be generated by evaluating
        a low-order fit to each image row, for both axes, and adding the two
        resulting images.
    ...
    for i in range(niter):
        ...
        if var is None:
            ...
            # Median filter the main data array only if it's used for
            # estimating cleaned values. The variance doesn't have background
            # subtracted, so no need to add it here.
            if cleantype == 'median':
                if sepmed:
                    m5_nobkg = sepmedfilt7(cleanarr)
                else:
                    m5_nobkg = medfilt5(cleanarr)
